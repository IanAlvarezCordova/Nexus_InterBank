...
public class CoreClient {
    ...
    public String iniciarDevolucion(java.util.Map<String, String> payload) {
        return webClient.post()
                .uri("/ventanilla/devoluciones")
                .bodyValue(payload)
                .retrieve()
                .onStatus(HttpStatusCode::isError, response -> response.bodyToMono(String.class)
                        .flatMap(error -> Mono.error(new RuntimeException(error))))
                .bodyToMono(String.class)
                .onErrorResume(e -> Mono.error(new RuntimeException(e.getMessage()))) // Propagate error message
                .block();
    }
}