...
public class CoreVentanillaController {
    ...
    private void guardarTransaccionVentanilla(TransaccionCajaRequest req, String tipo, String cuentaDestinoOverride) {
        Transaccion tx = new Transaccion();
        tx.setCuentaOrigen(req.getCuentaOrigen());
        tx.setCuentaDestino(cuentaDestinoOverride != null ? cuentaDestinoOverride : req.getCuentaDestino());
        ...
        if ("DEPOSITO".equalsIgnoreCase(tipo)) {
            ...
            tx.setRolTransaccion("DEBITO"); // desde origen la transacción es un débito
        }
        tx.setFechaEjecucion(java.time.LocalDateTime.now());
        // Valores técnicos mínimos
        tx.setInstructionId("VENT-" + System.currentTimeMillis());
        tx.setReferencia(tx.getInstructionId());
        tx.setIdBancoOrigen(2);
        tx.setIdBancoDestino(2);
