...
const PaginaDetalleCuenta = () => {
    ...
    const [selectedTxForReturn, setSelectedTxForReturn] = useState<MovimientoDTO | null>(null);
    ...
    const handleOpenReturn = (mov: MovimientoDTO) => {
        setSelectedTxForReturn(mov);
        setIsReturnModalOpen(true);
    };

    const handleConfirmReturn = async (reason: string) => {
        if (!selectedTxForReturn || !selectedTxForReturn.instructionId) return;

        setIsReturning(true);
        try {
            await bancaService.devolverTransaccion(selectedTxForReturn.instructionId, reason);
            toast.success('Solicitud de devolución enviada correctamente');
            setIsReturnModalOpen(false);
            // Optional: Refresh movements
        } catch (error: any) {
